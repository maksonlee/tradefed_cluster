...
class TestResourceParameters(ndb.Model):
  ...
  def FromMessage(cls, msg):
    return cls(decompress_files=msg.decompress_files)
...
def TestResourceToMessage(entity):
  ...
  return api_messages.TestResource(
      ...
      params=ToMessage(entity.params))
...
def RequestToMessage(request, command_attempts=None, commands=None):
  ...
  return api_messages.RequestMessage(
      ...
      type=request.type,
      user=request.user,
      command_line=request.command_infos[0].command_line,
      cluster=request.command_infos[0].cluster,
      run_target=request.command_infos[0].run_target,
      run_count=request.command_infos[0].run_count,
      shard_count=request.command_infos[0].shard_count,
